...
void GaussianBlurOnGPU(uchar4* const modifiedImage, const uchar4* const rgba, int rows, int cols, float* filter, int  filterWidth)
{
    ...
    checkCudaErrors(cudaMemPrefetchAsync(redBlurred, numPixels, 0, streams[0]));
    checkCudaErrors(cudaMemPrefetchAsync(red, numPixels, 0, streams[0]));
    
    ComputeConvolutionOnGPU <<<gridShape,blockShape,0, streams[0] >>>(redBlurred, red, dev_rows, dev_cols, dev_filter, dev_filterWidth);
    checkCudaErrors(cudaMemPrefetchAsync(redBlurred, numPixels, cudaCpuDeviceId, streams[0]));
    
    //segunda iter
    cudaStreamCreate(&streams[1]);
    checkCudaErrors(cudaMemPrefetchAsync(greenBlurred, numPixels, 0,streams[1]));
    checkCudaErrors(cudaMemPrefetchAsync(green, numPixels, 0, streams[1]));

    ComputeConvolutionOnGPU << <gridShape, blockShape,0, streams[1] >> > (greenBlurred , green , dev_rows, dev_cols, dev_filter, dev_filterWidth);
    checkCudaErrors(cudaMemPrefetchAsync(greenBlurred, numPixels, cudaCpuDeviceId, streams[1]));

    //tercera iter
    cudaStreamCreate(&streams[2]);
    checkCudaErrors(cudaMemPrefetchAsync(blueBlurred, numPixels, 0, streams[2]));
    checkCudaErrors(cudaMemPrefetchAsync(blue, numPixels, 0, streams[2]));
    ComputeConvolutionOnGPU << <gridShape, blockShape,0, streams[2] >> > (blueBlurred, blue, dev_rows, dev_cols, dev_filter, dev_filterWidth);
    checkCudaErrors(cudaMemPrefetchAsync(blueBlurred, numPixels, cudaCpuDeviceId, streams[2]));
